...
public class Ant
{
    ...
    */
    ...
    public ArrayList<Integer> getLengths(int currentNode, ArrayList<Integer> notVisited)
    {
        ...
        for (int node : notVisited) {
            lengths.add(sim.getEdgeLength(currentNode, node));
        }
        // END OF YOUR CODE
    ...
    */
    public ArrayList<Double> getLevels(int currentNode, ArrayList<Integer> notVisited)
    {
        ...
        // YOUR CODE GOES HERE - REPLACE THIS DUMMY CODE
        for (int node : notVisited) {
            levels.add(sim.getPheromoneLevel(currentNode, node));
        }
        // END OF YOUR CODE
    ...
    public ArrayList<Double> getProbabilities(ArrayList<Double> levels, ArrayList<Integer> lengths)
    {
        ...
        for (int i = 0; i < levels.size(); i++) {
            double pheromone = levels.get(i);
            double length = lengths.get(i);
            double component = Math.pow(pheromone, alpha) * Math.pow(1 / length, beta);
            total += component;
            components.add(component);
        }
        for (double component : components) {
            probabilities.add(component / total);
        }
        // END OF YOUR CODE
    ...
    */
    ...
    public int chooseNextNode(ArrayList<Double> cumulative)
    {
        ...
        for (int i = 0; i < cumulative.size(); i++) {
            if (random <= cumulative.get(i)) return i;
        }
        return 0;
        // END OF YOUR CODE
    }
